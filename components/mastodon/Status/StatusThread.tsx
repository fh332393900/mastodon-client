  ...
  return (
    <div className="flex gap-4">
      ...
      <div className="flex flex-col items-center">
        ...
        {/* Thread line below avatar — only when not last */}
        {!isLast && (
          <div className="mt-1 w-0.5 flex-1 min-h-4 bg-border/60 rounded-full" />
        )}
      </div>
      ...
      <div className={cn("min-w-0 flex-1 space-y-3", !isLast && "pb-4")}>
        ...
        <div className="[&_.prose]:max-w-none [&_.prose]:text-sm [&_.prose_a]:text-primary [&_.prose_p]:my-2">
          <MastodonContent
            content={renderedStatus.content}
            emojis={renderedStatus.emojis}
          />
        </div>
...
export function StatusThread({ statuses }: StatusThreadProps) {
  return (
    <article className="rounded-3xl border border-border/70 bg-card/90 p-4 shadow-sm space-y-0">
